...
在启动 Redis 之前，请确保已配置好环境变量文件 `.env`（位于 eino-examples/quickstart/eino_assistant 目录下），填写必要的大模型 API Key 等配置：

```bash
# 编辑 .env 文件，填入火山云方舟的模型接入点和 API Key
export ARK_API_KEY=your_api_key_here
export ARK_CHAT_MODEL=your_chat_model_endpoint
export ARK_EMBEDDING_MODEL=your_embedding_model_endpoint
```

> 注意：`.env` 文件使用了 `export` 语法，可直接通过 `source .env` 加载到当前 shell 环境，也会被应用程序自动读取。

完成配置后，直接用 redis 官方的 redis stack 镜像启动即可

```bash
# 切换到 eino_assistant 目录
cd xxx/eino-examples/quickstart/eino_assistant

# 加载环境变量（可选，运行程序时会自动读取）
source .env
